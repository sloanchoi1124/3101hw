...
int run_camera(Context& context)
{
	...
	Uint32 start_ticks = SDL_GetTicks();
        ...
        /* vectors to store previous frame, current frame and points on trajectory*/
	vector<uint8_t> prev_img, current_img_protected, current_img_copy;
	vector<uint8_t> trajectory_buffer(context.width * context.height * 3, 0);
        int counter = 0;
	while (ctx.running) {
        ...
        /* update previous frame from the unmodified copy of current frame*/
        prev_img = current_img_protected;
        /* get new frame; 
         * current_img_protected should not be modified, 
         * since it will be compared with the next frame*/
        current_img_protected = vector<uint8_t> ((uint8_t*) video_tex_pixels, (uint8_t*)video_tex_pixels+size);
        /* current_img_copy will be passed in to frame_function, and will be modified later*/
        current_img_copy = current_img_protected;
        counter++;
        /* apply frmae_function to current_img_copy and trajectory_buffer*/
        if (counter > 2)
        	frame_function(context, prev_img, current_img_copy, trajectory_buffer);        

        /* build a Mat with the updated current_img_copy */
        Mat m = Mat(480, 640, CV_8UC3, (uint8_t*)&current_img_copy[0]);
        /* build a Mat with the updated trajectory_buffer */
        Mat trajectory = Mat(480, 640, CV_8UC3, (uint8_t*)&trajectory_buffer[0]);
        Mat result;
        /* In trajectory_buffer, (255, 255, 255) indicates a point on the trajectory;
         * trajectory_buffer | current_img_copy will turn all points on the trajectory to (255, 255, 255)
         * */
        bitwise_or(m, trajectory, result);
